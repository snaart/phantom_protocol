...
| `cargo clippy --lib` warnings | **0** (was 41 — audit-todo cleanup landed `b9c7b57`: 6 mechanical fixes + 11 justified `#[allow]`s with `// PANIC-SAFETY:` comments) |
| **1.0 readiness** | Code-side roadmap **complete**. All 8 phases of the in-code work (0–7) closed or honestly deferred. The Phase 5 external-validation track (5.7) and Phase 6 optional formal-verification (6.10) are ⏭️ with rationale below. Cross-target CI is hard-gated on Linux × 4 + macOS × 2 + iOS × 2 + Windows × 2 + wasm32-unknown-unknown + thumbv7em-none-eabihf. wasm32-wasi remains the only known long-tail (scoped in `docs/operations/wasi.md`). |
...
| 3.4.9 | Docs — internal contributor map: legs list + conditional-compilation matrix | ✅ | _local_ | Internal contributor map updated with `SessionTransport`-relocation note (§1), `EmbeddedLeg` paragraph (§2), `embedded`-feature description in the features paragraph, build/test command, and Conditional Compilation Matrix bullet. The contributor map is `.gitignore`'d by project policy, so changes are not committed; the public-facing equivalents now live in `README.md` and `docs/architecture/ARCHITECTURE.md`. |
...
| 4.5 | Telemetry: `tracing` instrumentation (foundation) + metrics primitives | ✅ | _superseded by Phase 8_ | Original Phase 4.5 delivered `tracing` spans + hand-rolled `TransportMetrics` with Prometheus text exposition (`MetricsSnapshot::to_prometheus_text()`, `PhantomListener::metrics_prometheus_text()`). **Replaced wholesale by Phase 8 (OTel refactor)** — `transport::metrics` module and `metrics_prometheus_text` are gone; `phantom_core::observability` is the new public surface with OTLP/gRPC push. See Phase 8 row for delivery details. |
| 8.x | OpenTelemetry refactor (Observability) | ✅ | `feature/otel-observability` branch (commits `e59a1f1`..`5675ecc`) | Full pivot from hand-rolled Prometheus to OTel-native pipeline. New `phantom_core::observability` module: lock-free `HotPathAtomics` with `crossbeam_utils::CachePadded` (≤ 2.5 ns/call on M1, 84 ns/call contended × 8 threads), pre-interned attribute sets, `Counter` / `Histogram` (base-2 exponential) / `UpDownCounter` / `ObservableCounter` instruments under the configurable `phantom.*` namespace. `telemetry-otel` Cargo feature (off by default); ZST no-op `PhantomInstruments` when off — recording call sites collapse to atomic-only. `tracing-opentelemetry` bridge wires existing `phantom.*` spans into OTel traces; exemplar correlation on `phantom.handshake.duration` Histogram. New `server/src/telemetry.rs` installs OTLP/gRPC `MeterProvider` + `TracerProvider`; `server/src/metrics_http.rs` + hyper deps deleted. CI gate: `telemetry-otel-feature` job in `ci.yml`. Full design + atomic-commit rollout: `docs/observability/refactor-plan.md`. Deliverables: `docs/observability/{README,metrics-catalog,otlp-setup,tracing-guide}.md`, rewritten Grafana dashboard + Prometheus alerts under OTel-translated names, `examples/observability-demo` (docker-compose: OTel Collector + Prometheus + Tempo + Grafana). |
...
| 5.6 | Common Criteria Protection Profile mapping | ✅ | `18cb137` | `docs/compliance/cc-pp-mapping.md` (402 lines, 7 sections). Target PP: **NIAP PP-Module for VPN Client v2.5 + PP for Application Software v1.4**. **30 SFR rows** mapping FCS (CKM/COP/RBG/protocol-equivalence), FTP, FPT, FIA, FDP, FMT, FPR with `file:line` evidence pointers + status (✅/🔄/N/A). SAR mapping ties ADV_ARC→ARCHITECTURE.md, AGD_OPE→operations/, ATE_FUN→cavp + security_invariants + property tests, AVA_VAN→threat-model. 13 numbered gaps with remediation; biggest are POST self-tests (Phase 5.5 follow-on), `ring::LessSafeKey` zeroization (partial), and non-FIPS X25519/ChaCha20-Poly1305 (acceptable outside `fips` feature). Submission path documented (NIAP CCEVS, 6-12 months, accredited labs). |
| 5.7 | Validation pathway (CMVP / CC submission) | ⏭️ | — | **External — out of code scope.** Requires a CCEVS-accredited lab (Booz Allen, Leidos, etc.) engagement and a Security Target document grounded in `docs/compliance/cc-pp-mapping.md`. Code-side prerequisites (cc-pp-mapping, fips-readiness, cavp KATs, SLSA-3 provenance) all landed. Submission is a procurement+legal track, not a code track. |

**Phase 5 verdict:** **all code-side items closed.** ✅ 5.1, 5.2, 5.3, 5.4, 5.5, 5.6. ⏭️ 5.7 (external CMVP/CC lab validation — procurement+legal track, not code; prerequisites landed). FIPS-readiness deliverables complete: pure-Rust FIPS-203/204 primitives, constant-time audit, RNG audit, CAVP KATs, key-management + self-tests + security-policy + PP-mapping docs.
...
| 6.10 | Formal verification (ProVerif / Tamarin) | ⏭️ | — | **Optional / audit-driven.** Requires specialized symbolic-analysis tooling (ProVerif or Tamarin Prover) plus an analyst comfortable in applied-pi calculus. Not a code task — a separate research engagement. Phantom Core's design (transcript-bound handshake, constant-time replay window, sigstore-backed builds, 24 negative-security tests, threat model in `docs/security/threat-model.md`) provides the input artifacts; the analysis is downstream. Will revisit if/when a specific audit demands it. |
...
| 7.6 | Grafana dashboards + Prometheus alert rules | ✅ | _this commit; rewritten under Phase 8_ | Phase 7.6 originally shipped `docs/operations/grafana/phantom-dashboard.json` + `docs/operations/prometheus/alerts.yml` against the Phase-4.5 `phantom_*` names. **Phase 8 rewrote these** under the OTel-translated names; the canonical versions now live at `docs/observability/grafana/phantom-otel-dashboard.json` and `docs/observability/prometheus/alerts.yml`. The older Phase-4.5 files remain in `docs/operations/` for downstream embedders still on the legacy names — to be removed once they are no longer referenced. |
