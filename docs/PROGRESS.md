...
| Metric | Value |
| --- | --- |
| Tests passing | **238 / 238** (202 unit + 24 negative-security + 5 proptest + 3 fuzz + 1 alkahest + 1 runtime-integration; +2 tcp_integration `#[ignore]`) |
...
| 3.4 | `EmbeddedLeg` (UART / serial / CAN) | 🔄 | — | Sub-project **D** of the bare-metal-no_std track. `EmbeddedLeg<R, W, const N: usize>` over `embedded-io-async`, implementing the (relocated) `SessionTransport` trait — a peer of `TcpSessionTransport`. no_std+alloc-clean, built and unit-tested on `std`; bare-metal-green deferred to the no_std foundation (3.6). CAN is out of scope (`embedded-io-async` is byte-stream). Sub-items ↓ |
| 3.4.1 | Relocate `SessionTransport` → `transport/session_transport.rs` (the no_std transport boundary) | ⏳ | — | move trait out of std-bound `api/session.rs`; `pub use` re-export keeps every import path stable |
| 3.4.2 | `embedded` cargo feature + `embedded-io-async` / `embassy-sync` deps | ⏳ | — | both `optional`; `critical-section/std` for host tests; verify MSRV 1.75 |
| 3.4.3 | `framing.rs` — pure length-prefix logic + `FramingError` | ⏳ | — | `[u32 BE len][payload]`, byte-identical to `tcp_transport.rs`; I/O-free, fuzzable |
| 3.4.4 | `EmbeddedLeg<R, W, const N>` + `SessionTransport` impl | ⏳ | — | two `embassy-sync` mutexes (rx/tx) so a parked recv never blocks send; fixed `[u8; N]` recv buffer; passive (no internal task) |
| 3.4.5 | Wire `embedded` module into `legs/mod.rs` | ⏳ | — | `#[cfg(feature = "embedded")]`, mirroring the `websocket` gating |
| 3.4.6 | Tests — mock `embedded-io-async` duplex + unit + end-to-end handshake | ⏳ | — | round-trip, adversarial chunking, oversized/EOF, concurrent send/recv; e2e mirrors `test_phantom_session_handshake_via_transport` |
| 3.4.7 | `fuzz_embedded_framing` target | ⏳ | — | arbitrary bytes through the framing decode path → no panic/OOM |
| 3.4.8 | `embedded`-feature CI job | ⏳ | — | `cargo test/clippy --features embedded`; `thumbv7em` cross job stays `allow_failure` (that flip is sub-project E) |
| 3.4.9 | Docs — `CLAUDE.md` legs list + conditional-compilation matrix | ⏳ | — | finalize these sub-items to ✅ |
