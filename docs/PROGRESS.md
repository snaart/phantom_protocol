...
| 3.6 | no_std + `alloc` for embedded | 🔄 | `e0486c7` | Foundation laid: the embedded transport subset (`transport::session_transport`, `transport::legs::embedded::{mod,framing}`, `errors::CoreError`) is verifiably no_std + alloc-clean — proven via a vendored isolation probe that compiles cleanly on `--target thumbv7em-none-eabihf`. New `no-std` Cargo feature (off by default) acts as an **audit gate**: per-module `#![cfg_attr(feature = "no-std", no_std)]` ensures these files don't accidentally pull `std::*`. The original prerequisite (no C deps — swap `pqcrypto-*` → `ml-kem`/`ml-dsa`) was done in Phase 5.1. Whole-crate `cargo check --target thumbv7em-none-eabihf` still fails on transitive std-deps (`once_cell`, `subtle`, `memchr`, `getrandom`) pulled by the std-bound modules (`api`, `runtime`, the other transport legs); flipping to ✅ requires gating each of them behind `#[cfg(feature = "std")]` — multi-week structural refactor. Bricks laid; wall pending. |
...
**Phase 3 verdict:** ✅ 3.1 (Runtime trait), 3.2 (Clock folded into Runtime), 3.3 (WebSocketLeg + WasmRuntime + wasm32 build green), 3.4 (EmbeddedLeg), 3.5 (conditional-compilation matrix — wasm32-unknown-unknown is now a hard CI gate), 3.7 (compression-zstd optional), 3.8 (RNG abstraction — `RngProvider` trait + `OsRng` + documented slot-ins for hardware-RNG / DRBG; downstream HAL adapters pick up the hooks), 3.10 (WASM-specific tweaks — tokio split + getrandom wasm_js fix), 3.11 (cross CI matrix). 🔄 3.6 (no_std + alloc for embedded — subset foundation laid via `e0486c7` and verified on thumbv7em via isolation probe; widening to whole-crate `--target thumbv7em-none-eabihf`-green requires gating `pub mod api`/`pub mod runtime`/std-bound transport legs behind a `std` feature — structural refactor). ⏳ 3.9 (Swift/Kotlin/C FFI bindings — only Python ships). wasm32-wasi target remains `allow_failure: true` in CI pending a wasi-flavored leg story (separate long-tail item).
...
| 7.2 | Deployment guides (Docker / k8s / systemd / mobile / WASM) | 🔄 | `cab6ca1` + `24ac3aa` | `docs/operations/docker.md` (Dockerfile, ulimits, metrics endpoint, graceful-shutdown), `docs/operations/systemd.md` (unit file + hardening + multi-instance SO_REUSEPORT template + sysctl), `docs/operations/deployment.md` (overview + pre-deploy checklist + capacity planning), `docs/operations/kubernetes.md` (`cab6ca1` — Deployment + Service + probes + Secret-not-ConfigMap for HybridSigningKey + PDB w/ graceful-shutdown + HPA on phantom_* metrics + NetworkPolicy + rolling-update strategy + SO_REUSEPORT clarification + monitoring/logging), `docs/operations/wasm.md` (`24ac3aa` — first client-side guide: WebSocketLeg + WasmRuntime wiring, server-key pinning via `include_bytes!`, IndexedDB resumption-ticket pattern, bundle-size + Worker + observability + security caveats). Mobile guide + Helm chart still pending. |
...
**Phase 7 verdict:** ✅ 7.3, 7.4, 7.5, 7.6, 7.7, 7.8. 🔄 7.1 (loopback example landed; mobile / WASM / embedded example crates pending — wasm32 is now buildable so an `examples/wasm_browser_demo.rs` can land), 7.2 (Docker / systemd / k8s / wasm / deployment-index guides landed via `cab6ca1` + `24ac3aa`; mobile guide + Helm chart still pending). 7.4 closed via `fb89465` — SLSA-3 OIDC provenance attestation integrated. SLSA L4 (hermetic builds) is a separate long-tail item.
