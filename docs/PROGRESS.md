...
**Phase 3 verdict:** **10 of 11 items closed.** ✅ 3.1 (Runtime trait), 3.2 (Clock folded into Runtime), 3.3 (WebSocketLeg + WasmRuntime + wasm32 build green), 3.4 (EmbeddedLeg), 3.5 (conditional-compilation matrix), 3.6 (no_std + alloc — `thumbv7em-none-eabihf` bare-metal build now green via `--no-default-features --features embedded,no-std`; CI hard-gated in `b273ca3`), 3.7 (compression-zstd optional), 3.8 (RNG abstraction — `RngProvider` trait + slot-ins for hardware-RNG / DRBG), 3.10 (WASM-specific tweaks), 3.11 (cross CI matrix — both wasm32-unknown-unknown and thumbv7em-none-eabihf are hard gates). ⏳ 3.9 (Swift/Kotlin/C FFI bindings — only Python ships). wasm32-wasi target remains `allow_failure: true` in CI pending a wasi-flavored leg story (separate long-tail item). Future widening of the no_std-clean subset (e.g., promoting `crypto/kdf.rs`, `security/*`, parts of `transport/*` out of the `std`-gated region) can land module-by-module without re-touching the gating infrastructure landed in `722c6cb`.
