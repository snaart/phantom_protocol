...
| Metric | Value |
| --- | --- |
| Tests passing | **250 / 250** (214 unit + 24 negative-security + 5 proptest + 3 fuzz + 1 alkahest + 1 runtime-integration; +2 tcp_integration `#[ignore]`) — `+12` unit from Phase 3.4 embedded leg |
...
| 3.4.6 | Tests — mock `embedded-io-async` duplex + unit + e2e | ✅ | `809707f` + `f7bcfb1` | `Tokio Notify`-driven mock duplex (`MockReader` / `MockWriter` over `Pipe`) in `809707f` + 7 unit tests (round-trip x2 + 5 framing). `f7bcfb1` added 5 more: adversarial chunking (`max_read=1` per `read` call), oversized header reject, EOF mid-header, concurrent send/recv (1 s timeout), and `SessionTransport`-trait-surface round-trip. **12 embedded tests total, all pass on `--features embedded`.** Full `test_phantom_session_handshake_via_transport` mirror over `EmbeddedLeg` (needs `HybridSigningKey` + handshake-driver scaffolding) remains an optional future addition. |
