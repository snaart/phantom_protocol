...
| **L**inkability of one session across a network change (migration) | **Mitigated (ε) — client migration unlinkable both ways; server-migration c2s residual** | Header protection (T4.6, §4.6) masks the variable per-packet metadata (packet numbers, flags incl. PRIORITY, stream id, epoch, path id), and ε removed the inner 32-byte `session_id` from the wire (off-wire in the AEAD AAD — §4.2) and makes the routing `ConnId` **rotate** per migration via per-direction KDF chains + a sliding demux window (§4.7). A **client** migration rotates **both** directions (EPS-02 fix, audit 2026-06-15): the client advances its c2s chain on `migrate()`, and the server advances its s2c chain on authenticating the new `path_id` (post-AEAD), the socket-routed client absorbing the new inbound CID with no window slide and no ping-pong. So a client moving Wi-Fi→cellular is unlinkable in both directions. **Residual:** a *server*-initiated migration rotates s2c (its own `migrate()`) but **not** c2s — the socket-routed client does not rotate-on-detect (that would push its CID out of the server's un-sliding c2s window for repeated server migrations), so a server move stays c2s-linkable to a both-networks observer; server migration is rare and the full shared-epoch fix is tracked (`docs/security/audit-report-2026-06-15-wire-v5-epsilon.md`). **Honest caveat:** the CID chain is session-stable and **not** forward-secret — a session-key compromise lets an attacker recompute the chain and relink a *recorded* flow; the payload stays forward-secret. The constant `version` byte is a protocol (not per-connection) fingerprint — full anti-fingerprinting is a separate future pass. See `PROTOCOL.md` §4.2 / §4.7 / §12.5. |
...
- Connection migration with path validation (Phase 4) is SHIPPED (P4.0-P4.4):
  ...
  CID rotate per migration. A **client** migration rotates **both** directions
  (EPS-02 fix): the client rotates c2s on `migrate()` and the server rotates s2c on
  authenticating the new path_id, so a client move is unlinkable both ways. Residual:
  a *server* migration rotates s2c but not c2s (the socket-routed client does not
  rotate-on-detect), so a server move stays c2s-linkable — rare; the shared-epoch
