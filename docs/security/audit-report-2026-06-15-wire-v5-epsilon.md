...
**Regression test:** `eps01_window_strands_on_multistep_migration_jump` in `core/tests/security_invariants.rs` — register a server window at `highest_seen = 0`, advance the *client* outbound index by 6, drive a single `note_migration_path(6)`, and assert `inbound_window_cids()` does **not** yet contain `client.outbound_cid(6)` (pins the current boundary; flips to *contains* once the multi-step slide lands). Added with its fix in the follow-up migration-redesign PR.
...
**Regression test:** `eps02_server_rotates_s2c_cid_on_client_migration` in `core/tests/security_invariants.rs` — after a client `note_migration_path`/migrate is observed by the server, assert `server.current_outbound_cid()` has changed from `CID_s2c(0)` (RED today; GREEN after the fix). Plus an integration assertion that both directions' on-wire CIDs differ pre- vs post-migration. Added with its fix in the follow-up migration-redesign PR.
...
2. **[follow-up PR] Symmetric & robust migration — close EPS-01 + EPS-02 together.** Single authenticated migration epoch `E`: `migrate()` increments it; recv catches `E` up to the authenticated `path_id`, rotating **both** directions' CIDs and multi-step-sliding the inbound window. Full manual TDD, live recv-only + byte-exact migration tests, then flip `eps01_*` / `eps02_*` to GREEN and **restore** the § 12.5 / threat-model "closed" claim.
3. **[follow-up PR] Cosmetic: replace the dead `session_id` backstop (EPS-07) with a `debug_assert_eq!` + the cross-delivery negative test, alongside the migration work that already touches `handle_packet`.**

Until item 2 lands, § 12.5 / threat-model § A6 / LINDDUN-L are corrected in this PR to state the two honest residuals (the s2c direction is linkable across a client migration; migration tolerates at most K in-flight un-acked generations).
