...
**Status (CLOSED for *both* migration directions — A2a, 2026-06-17):** the server rotates its s2c CID on authenticating the client's new `path_id` (post-AEAD) — closing the client-migration case — AND, with A2a, server-initiated migration is now a real symmetric feature that closes the residual: when the server migrates (`migrate_server`, rotating s2c + bumping its `path_id`), the client **reflects** on authenticating the server's new `path_id` — it bumps its OWN `path_id` and rotates its c2s chain. That `path_id` bump is precisely what makes the server slide its c2s demux window to the rotated CID (so it stays routable — the *real* fix for the stranding the original "client must not rotate" guard avoided, which was caused by rotating the CID *without* a path_id step). The server's matching s2c re-rotation is `path_id`-silent, so the client sees no new forward server `path_id` and does not re-reflect — it terminates in one round (no ping-pong); and the client's reflected c2s comes from its unchanged source, so the server's `confirm_authenticated_source` is a no-op for it. This is the per-direction-coupled equivalent of the proposed single-epoch `E` (the two indices stay distinct but advance in lock-step via reflection); it does not, by itself, close EPS-01, which is fixed independently above. Anti-spoof on the client mirrors the server (M-1 candidate-from-authenticated-source + 3× anti-amp + promote-only-on-valid-echo). **LINDDUN-L is now closed for migration by either peer.** Tests: `eps02_client_rotates_c2s_on_server_migration` (inverted from the old guard), the anti-amp negative test in `security_invariants.rs`, and the live `udp_integration_server_migration_rotates_both_cids_and_survives` (both CIDs rotate on the wire across a server migration, byte-exact). The session-stable, not-forward-secret CID-chain caveat is unchanged.
