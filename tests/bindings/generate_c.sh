#!/usr/bin/env bash
#
# generate_c.sh — regenerate / verify the C FFI header for phantom_core.
#
# UniFFI 0.29 has no first-class C generator, so the header at
# tests/bindings/c/phantom_core.h is hand-curated against the exact
...
HEADER="$ROOT/tests/bindings/c/phantom_core.h"
...
if [[ $SKIP_BUILD -eq 0 ]]; then
    echo "==> Building libphantom_core ($PROFILE)..."
...
for ext in dylib so dll; do
    cand="$ROOT/target/$PROFILE/libphantom_core.$ext"
    [[ -f "$cand" ]] && { DYLIB="$cand"; break; }
done
# Windows convention drops the lib prefix
if [[ -z "$DYLIB" ]]; then
    cand="$ROOT/target/$PROFILE/phantom_core.dll"
    [[ -f "$cand" ]] && DYLIB="$cand"
fi

if [[ -z "$DYLIB" ]]; then
    echo "could not find a built libphantom_core under $ROOT/target/$PROFILE" >&2
...
if [[ "$(uname -s)" == "Darwin" ]]; then
    nm -gU "$DYLIB" \
        ...
        | grep -E '^(uniffi_phantom_core|ffi_phantom_core)_' \
        | sort -u > "$SYMBOLS_FILE"
else
...
# We look for any token matching ^(uniffi|ffi)_phantom_core_\w+$ inside
...
grep -oE '\b(uniffi_phantom_core|ffi_phantom_core)_[A-Za-z0-9_]+\b' "$HEADER" \
    | sort -u > "$HEADER_SYMS"
...
language = "C"
include_guard = "PHANTOM_CORE_CBINDGEN_CONSTS_H"
