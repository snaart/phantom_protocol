# Phantom Core — binding packaging
...
| C      | release tarball + pkg-config | `c/phantom_core.pc.in`, `c/package.sh` |
...
The wheel bundles `phantom_core.py`; the native library is platform-specific
...
The wheel produced by `python -m build` includes `phantom_core.py` but does
...
./build-xcframework.sh         # produces PhantomCore.xcframework
...
upload `PhantomCore.xcframework.zip` to the GitHub Release.
...
../generate_kotlin.sh          # regenerates uniffi/phantom_core/phantom_core.kt
gradle :assembleRelease        # produces build/outputs/aar/phantom_core-release.aar
...
# or
./package.sh --prefix /opt/phantom_core
# Output: phantom_core-c-<version>-<os>-<arch>.tar.gz in $PWD
```

Run on every OS / arch you intend to publish for — the tarball bundles
the host's prebuilt `libphantom_core.{dylib,so,dll}`. Attach the tarballs
to a GitHub Release.
...
- `tests/bindings/c/phantom_core.pc.in` (`Version: ...`)
